import * as React from 'npm:react@18.3.1'
import {
  Body,
  Button,
  Container,
  Head,
  Heading,
  Hr,
  Html,
  Preview,
  Section,
  Text,
} from 'npm:@react-email/components@0.0.22'
import type { TemplateEntry } from './registry.ts'

interface CourseAccessVipProps {
  name?: string
  accessUrl?: string
  planLabel?: string
  buyerEmail?: string
  whatsappNumber?: string
}

export const CourseAccessVipEmail = ({
  name,
  accessUrl = 'https://www.skool.com/leo-avatar-method-9184',
  planLabel = 'Formation + accompagnement',
  buyerEmail,
  whatsappNumber = '07 67 98 43 21',
}: CourseAccessVipProps) => (
  <Html lang="fr">
    <Head />
    <Preview>Ton accès + ton accompagnement WhatsApp avec moi — utilise le MÊME email qu'à l'achat</Preview>
    <Body style={body}>
      <Container style={container}>
        <Heading style={h1}>Bienvenue{name ? ` ${name}` : ''} 👋</Heading>
        <Text style={text}>
          Ton paiement est confirmé. Tu as maintenant accès à <strong>{planLabel}</strong>, avec
          mon accompagnement personnalisé.
        </Text>
        <Section style={{ textAlign: 'center', margin: '32px 0' }}>
          <Button href={accessUrl} style={button}>
            Accéder à la formation
          </Button>
        </Section>
        <Text style={muted}>
          Si le bouton ne fonctionne pas, copie ce lien dans ton navigateur :<br />
          {accessUrl}
        </Text>

        <Section style={vipBox}>
          <Text style={vipTitle}>📱 Ton accompagnement WhatsApp</Text>
          <Text style={vipNumber}>{whatsappNumber}</Text>
          <Text style={text}>
            Enregistre mon numéro et envoie-moi un message avec ton prénom et ton email d'achat.
            C'est moi qui te réponds, jamais un associé, sous 2 h maximum.
          </Text>
          <Text style={text}>
            Ce que tu as avec l'accompagnement : <strong>1 appel par semaine</strong> avec moi,
            la <strong>relecture à vie de tes scripts TikTok</strong> (je relis quand tu veux),
            l'audit de ta niche, de ton offre et de ton avatar IA.
          </Text>
        </Section>

        <Section style={warnBox}>
          <Text style={warnTitle}>⚠️ TRÈS IMPORTANT</Text>
          <Text style={warnBig}>UTILISE LE MÊME EMAIL QUE CELUI DE TON ACHAT</Text>
          {buyerEmail ? <Text style={warnEmail}>{buyerEmail}</Text> : null}
          <Text style={warnText}>
            Quand tu cliques sur « Accéder à la formation » puis sur <strong>REJOINDRE</strong>,
            Skool te posera une question :<br />
            <strong>« Quel est l'email avec lequel vous avez acheté la formation ? »</strong>
          </Text>
          <Text style={warnText}>
            Tu dois répondre avec <strong>exactement le même email</strong> que celui utilisé pour
            le paiement. Si l'email est différent, <strong>la demande sera refusée</strong>.
          </Text>
        </Section>

        <Hr style={hr} />
        <Text style={text}>
          Mon conseil : commence par le module 1 dès aujourd'hui et écris-moi sur WhatsApp pour
          qu'on cale ton premier appel.
        </Text>
        <Text style={muted}>
          Une question ? Réponds simplement à cet e-mail ou écris à velascozorzi@gmail.com.
        </Text>
      </Container>
    </Body>
  </Html>
)

const body = { backgroundColor: '#0b0a12', fontFamily: 'Helvetica, Arial, sans-serif' }
const container = { margin: '0 auto', padding: '32px 24px', maxWidth: '560px' }
const h1 = { color: '#ffffff', fontSize: '26px', fontWeight: '700' as const }
const text = { color: '#d8d5e6', fontSize: '15px', lineHeight: '24px' }
const muted = { color: '#8e8aa3', fontSize: '13px', lineHeight: '20px' }
const button = {
  backgroundColor: '#6c47ff',
  color: '#ffffff',
  borderRadius: '10px',
  padding: '14px 28px',
  fontSize: '15px',
  fontWeight: '600' as const,
  textDecoration: 'none',
}
const hr = { borderColor: '#252238', margin: '28px 0' }
const vipBox = {
  backgroundColor: '#12102a',
  border: '2px solid #34d399',
  borderRadius: '12px',
  padding: '20px 20px 8px',
  margin: '32px 0',
}
const vipTitle = {
  color: '#34d399',
  fontSize: '16px',
  fontWeight: '700' as const,
  margin: '0 0 8px',
  textAlign: 'center' as const,
}
const vipNumber = {
  color: '#ffffff',
  fontSize: '28px',
  fontWeight: '800' as const,
  textAlign: 'center' as const,
  letterSpacing: '1px',
  margin: '0 0 16px',
}
const warnBox = {
  backgroundColor: '#1a1030',
  border: '2px solid #6c47ff',
  borderRadius: '12px',
  padding: '20px 20px 8px',
  margin: '32px 0',
}
const warnTitle = {
  color: '#ffb020',
  fontSize: '16px',
  fontWeight: '700' as const,
  margin: '0 0 8px',
  textAlign: 'center' as const,
}
const warnBig = {
  color: '#ffffff',
  fontSize: '26px',
  lineHeight: '32px',
  fontWeight: '800' as const,
  textAlign: 'center' as const,
  textTransform: 'uppercase' as const,
  margin: '0 0 12px',
}
const warnEmail = {
  color: '#c9b9ff',
  fontSize: '18px',
  fontWeight: '700' as const,
  textAlign: 'center' as const,
  margin: '0 0 16px',
  wordBreak: 'break-all' as const,
}
const warnText = { color: '#d8d5e6', fontSize: '15px', lineHeight: '24px' }

export const template = {
  component: CourseAccessVipEmail,
  subject: 'Ton accès + ton accompagnement WhatsApp 🚀 (utilise le même email)',
  displayName: 'Accès formation + accompagnement',
  previewData: {
    name: 'Léo',
    buyerEmail: 'client@exemple.com',
    accessUrl: 'https://www.skool.com/leo-avatar-method-9184',
    planLabel: 'Formation + accompagnement',
    whatsappNumber: '07 67 98 43 21',
  },
} satisfies TemplateEntry
